...
function CrmClients({ salonId, salonName }: { salonId: string; salonName: string }) {
  ...
  function shareRating() {
    ...
    const medals = ["🥇","🥈","🥉"];
    const lines = list.slice(0,10).map((c,i)=>`${medals[i]??`${c.rank}.`} ${c.name} — ${t("visitsCount", { count:c.total_visits })}`).join("\n");
    const text = `${t("crm.rating.shareHeader", { salon:salonName })}\n\n${lines}\n\n${t("crm.rating.shareFooter")}`;
    ...
    <div style={{ padding:"32px 36px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
        ...
        <button onClick={handleExport} disabled={exporting} style={{ display:"flex", alignItems:"center", gap:6, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", color:"var(--text2)", padding:"9px 16px", fontSize:13, fontWeight:500, cursor:exporting ? "not-allowed" : "pointer", opacity:exporting ? 0.6 : 1 }}>
          <Download size={14} /> {exporting ? t("crm.exportingButton") : t("crm.exportButton")}
        </button>
      </div>
      ...
      <div style={{ display:"flex", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:4, gap:4, width:"fit-content", marginBottom:20 }}>
        {(["list","rating"] as Tab[]).map((tb) => (
          <button key={tb} onClick={() => setTab(tb)} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 16px", borderRadius:9, border:"none", cursor:"pointer", fontSize:13, fontWeight:500, background: tab===tb ? "var(--gold)" : "transparent", color: tab===tb ? "#0a0a0b" : "var(--text2)" }}>
            {tb === "rating" && <Trophy size={13} />}
            {tb === "list" ? t("crm.tabs.list") : t("crm.tabs.rating")}
          </button>
        ))}
      </div>
      ...
      {tab === "list" ? (
          ...
          {hasNextPage && (
            <div style={{ display:"flex", justifyContent:"center", marginTop:20 }}>
              <button onClick={() => fetchNextPage()} disabled={isFetchingNextPage} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", color:"var(--text2)", padding:"9px 24px", fontSize:13, cursor:isFetchingNextPage ? "not-allowed" : "pointer", opacity:isFetchingNextPage ? 0.6 : 1 }}>
                {isFetchingNextPage ? tCommon("loading") : t("crm.loadMore")}
              </button>
            </div>
          )}
        </>
        ...
        <>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
            <p style={{ color:"var(--text2)", fontSize:13 }}>{t("crm.rating.subtitle")}</p>
            <button onClick={shareRating} disabled={(loyalty?.items.length ?? 0) === 0} style={{ display:"flex", alignItems:"center", gap:6, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", color:"var(--text2)", padding:"7px 14px", fontSize:13, cursor:"pointer", opacity:(loyalty?.items.length ?? 0) === 0 ? 0.4 : 1 }}>
              ...
              {copied ? t("crm.rating.copiedButton") : t("crm.rating.shareButton")}
            </button>
          </div>
          {loyaltyLoading ? <CrmSkeleton /> : (loyalty?.items.length ?? 0) === 0 ? <EmptyState text={t("crm.rating.emptyState")} /> : (
  ...
  return (
    <div style={{ padding:"32px 36px" }}>
      ...
      <div style={{ position:"relative", marginBottom:16 }}>
        <Search size={14} style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color:"var(--text3)", pointerEvents:"none" }} />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("searchPlaceholder")} style={{ width:"100%", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:"9px 14px 9px 36px", fontSize:13, color:"var(--text)", outline:"none", boxSizing:"border-box" }} />
      </div>

      {isLoading && <CrmSkeleton />}
      {!isLoading && items.length === 0 && <EmptyState text={debounced ? t("emptyState.notFound") : t("emptyState.noClients")} />}
      ...
      {showCreate && (
        <ModalBox title={t("personal.modal.createTitle")} onClose={() => setShowCreate(false)}>
          <ClientForm form={form} setForm={setForm} formErr={formErr} onSubmit={() => { setFormErr(""); createM.mutate({ name:form.name, phone:form.phone, notes:form.notes||undefined }); }} onCancel={() => setShowCreate(false)} loading={createM.isPending} submitLabel={t("personal.form.createSubmit")} />
        </ModalBox>
      )}

      {editing && (
        <ModalBox title={t("personal.modal.editTitle")} onClose={() => setEditing(null)}>
          <ClientForm form={form} setForm={setForm} formErr={formErr} onSubmit={() => { setFormErr(""); updateM.mutate({ id:editing.id, body:{ name:form.name, phone:form.phone, notes:form.notes||undefined } }); }} onCancel={() => setEditing(null)} loading={updateM.isPending} submitLabel={tCommon("save")} />
          <div style={{ marginTop:12, paddingTop:12, borderTop:"1px solid var(--border)" }}>
            <button onClick={() => { if (confirm(t("personal.deleteConfirm", { name:editing.name }))) deleteM.mutate(editing.id); }} disabled={deleteM.isPending} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", color:"var(--red)", fontSize:13, padding:"8px 0", opacity:deleteM.isPending ? 0.5 : 1 }}>
              {deleteM.isPending ? t("personal.deletingButton") : t("personal.deleteButton")}
            </button>
          </div>
        </ModalBox>
      )}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      {(["name","phone"] as const).map((field) => (
        <div key={field}>
          <label style={{ display:"block", fontSize:11, color:"var(--text2)", marginBottom:6 }}>{field === "name" ? t("personal.form.name") : t("personal.form.phone")}</label>
          <input value={form[field]} onChange={(e) => setForm((f) => ({ ...f, [field]:e.target.value }))} placeholder={field === "name" ? t("personal.form.namePlaceholder") : t("personal.form.phonePlaceholder")} style={fldInp} />
        </div>
      ))}
      <div>
        <label style={{ display:"block", fontSize:11, color:"var(--text2)", marginBottom:6 }}>{t("personal.form.notes")}</label>
        <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes:e.target.value }))} placeholder={t("personal.form.notesPlaceholder")} rows={2} style={{ ...fldInp, resize:"none" }} />
      </div>
      ...
      <div style={{ display:"flex", gap:10 }}>
        <button onClick={onCancel} style={{ flex:1, background:"var(--bg)", border:"1px solid var(--border)", borderRadius:"var(--radius)", color:"var(--text2)", fontSize:13, padding:"9px", cursor:"pointer" }}>{tCommon("cancel")}</button>
        <button onClick={onSubmit} disabled={loading || !form.name || !form.phone} style={{ flex:1, background:"var(--gold)", color:"#0a0a0b", border:"none", borderRadius:"var(--radius)", fontSize:13, fontWeight:700, padding:"9px", cursor:(loading || !form.name || !form.phone) ? "not-allowed" : "pointer", opacity:(loading || !form.name || !form.phone) ? 0.5 : 1 }}>
          {loading ? t("personal.form.submitting") : submitLabel}
        </button>
      </div>
    </div>
  );
